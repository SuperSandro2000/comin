...
func TestDeployerBasic(t *testing.T) {
	...
	g := &protobuf.Generation{SelectedCommitId: "commit-1", OutPath: "out-path"}
	d.Submit(g, "test")
...
func TestDeployerSubmit(t *testing.T) {
	...
	d.Submit(&protobuf.Generation{SelectedCommitId: "commit-1", OutPath: "out-path"}, "test")
	assert.EventuallyWithT(t, func(c *assert.CollectT) {
		assert.True(c, d.IsDeploying())
		assert.Nil(c, d.GenerationToDeploy)
	}, 5*time.Second, 100*time.Millisecond)

	d.Submit(&protobuf.Generation{SelectedCommitId: "commit-2", OutPath: "out-path"}, "test")
	d.Submit(&protobuf.Generation{SelectedCommitId: "commit-3", OutPath: "out-path"}, "test")
